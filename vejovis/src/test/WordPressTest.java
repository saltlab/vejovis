...
public class WordPressTest {
	...
	static final String JSSOURCEOUTPUTFOLDER = "js_files_wordpress";
	static final String EXECUTIONTRACEDIRECTORY = "executiontrace/";
	static final String DOMIDLISTSDIRECTORY = "domIdLists/";
	static final String OUTPUTDIRECTORY = "/wordpress_assertions/";
	static final String INJECTIONSFOLDER = "wordpressInjections/";
	...
	public static void main(String[] args) {
		...
		String injectModSrc = "";
		URL = "http://192.168.1.84/wp/wp-admin/comment.php?action=editcomment&c=1";
		//URL = "http://192.168.1.84/wp/wp-admin/edit-comments.php";
		int numActivations = 150;
		int injStart = 0;
		
		//Redirect System.out to file
		File file = new File("/wordpressInjections/results_150-21.txt");
	...
	private static void test1() {
		...
		crawler.click("A").withAttribute("href", "edit-comments.php");
		//crawler.click("A").withAttribute("href", "http://192.168.1.84/wp/wp-admin/post.php?post=1&action=edit");
		//crawler.click("A").withAttribute("id", "edButtonHTML");
		crawler.click("TD").withAttribute("class", "comment column-comment");
		crawler.click("A").withAttribute("id", "ed_reply_strong");
		crawler.click("A").withAttribute("class", "vim-r");
		
		//Login
		InputSpecification input = new InputSpecification();
		input.field("user_login").setValue("focarizajr");
		input.field("user_pass").setValue("legend");
		crawler.click("input").withAttribute("value", "Log In");
		crawler.setInputSpecification(input);
		
		//Exclude list
		List<String> wordpressExclude = new ArrayList<String>();
		wordpressExclude.add(".*php.*");
		wordpressExclude.add(".*l10n.*");
		wordpressExclude.add(".*ui.core.js.*");
		wordpressExclude.add(".*sortable.*");
		wordpressExclude.add(".*suggest.*");
		wordpressExclude.add(".*autosave.*");
		wordpressExclude.add(".*wp-langs-en.*");
		wordpressExclude.add(".*popup.*");
		wordpressExclude.add(".*thickbox.*");
		wordpressExclude.add(".*position.*");
		wordpressExclude.add(".*draggable.*");
		wordpressExclude.add(".*button.*");
		wordpressExclude.add(".*dialog.*");
		wordpressExclude.add(".*tinymce.*");
		
		
		excludeList = wordpressExclude;
