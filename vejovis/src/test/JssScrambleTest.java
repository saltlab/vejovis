...
public class JssScrambleTest {
	private static String URL = "";
	
	static CrawljaxConfiguration config;
	static CrawlSpecification crawler;
	static List<String> excludeList;
	static List<String> funcExcludeList;
	static String outputFolder;
	
	static final String JSSOURCEOUTPUTFOLDER = "js_files_jssscramble";
	static final String EXECUTIONTRACEDIRECTORY = "executiontrace/";
	static final String DOMIDLISTSDIRECTORY = "domIdLists/";
	static final String OUTPUTDIRECTORY = "/jssscramble_assertions/";
	static final String INJECTIONSFOLDER = "jssScrambleInjections/";
	...
	public static void main(String[] args) {
		...
		String injectModSrc = "";
		URL = "http://frolinsfilms.comuv.com/scramble.html";
		int numActivations = 150;
		int injStart = 166;
		
		//Redirect System.out to file
		File file = new File("/jssScrambleInjections/results_150-10.txt");
		...
		for (int i = injStart; i < injList.size(); i++) {
			...
			try {
				...
				//modifier.setExcludeFunctionsList(funcExcludeList);
				/*START INJECT MODE*/
				if (injectMode) {
					modifier.setInject();
	...
	private static void test1() {
		...
		crawler.click("A").withAttribute("class", "scramble");
		crawler.click("INPUT").withAttribute("type", "button");
		
		//Exclude list
		List<String> jssScrambleExclude = new ArrayList<String>();
		
		excludeList = jssScrambleExclude;
